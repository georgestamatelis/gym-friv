...
WINDOW_W=500
WINDOW_H=500
class zombieOnslaughtHardEnv(gym.Env):
    metadata = {
    ...
    def reset(self):
        ...
        self.weakZombies.append(weakZombie(880,self.positions[0],64,64))
        self.weakZombies.append(weakZombie(880,self.positions[1],64,64))
        self.weakZombies.append(weakZombie(880,self.positions[2],64,64))

        #strong zombies 
        self.strongZombies.append(strongZombie(700,self.positions[0],64,64))
        self.strongZombies.append(strongZombie(700,self.positions[1],64,64))
        self.strongZombies.append(strongZombie(700,self.positions[2],64,64))
        
        self.strongZombies.append(strongZombie(740,self.positions[1],64,64))
        self.strongZombies.append(strongZombie(770,self.positions[1],64,64))
        self.strongZombies.append(strongZombie(800,self.positions[1],64,64))

        self.strongZombies.append(strongZombie(910,self.positions[0],64,64))
        self.strongZombies.append(strongZombie(910,self.positions[1],64,64))
        self.strongZombies.append(strongZombie(910,self.positions[2],64,64))
            
        self.strongZombies.append(strongZombie(930,self.positions[1],64,64))
        

        self.zombiesToKill=len(self.strongZombies)+len(self.weakZombies)
        self.shootReset=0
        self.run = True
    ...
    def step(self,action):
        ...
        """
        first check bullet collision with weak zombies
        """
        for bullet in self.bullets:
            ...
            for z in self.weakZombies:
                ...
                if pygame.Rect.colliderect(rectA,rectB)==True:
                    ...
                    self.bullets.pop(self.bullets.index(bullet))
        """
        Now check bullet collision with strong zombies
        """
        for bullet in self.bullets:
            for sz in self.strongZombies:
                rectA=pygame.Rect(sz.hitbox)
                rectB=pygame.Rect(bullet.hitbox)
                if pygame.Rect.colliderect(rectA,rectB)==True:
                    sz.HP-=50
                    if sz.HP<=0:
                        self.strongZombies.remove(sz)
                        self.zombiesKilled+=1
                        reward+=1.0/self.zombiesToKill
        ...
        """
        now check collision between weak zombies and crates
        """
        for wz in self.weakZombies:
            ...
            if wz.x<=0:
                ...
                return state,reward,done,{}
        """
        now check for collision between strong zombies and crates
        """
        for sz in self.strongZombies:
            for cr in self.Crates:
                if cr.manCollides(sz):
                    cr.HP-=50
                    sz.vel=0
                    if cr.HP<=0:
                        self.Crates.remove(cr)
            ...
            if sz.x<=0:
                ...
                return state,reward,done,{}
        """
        now move the soldier based on the actions given by the player
        """
